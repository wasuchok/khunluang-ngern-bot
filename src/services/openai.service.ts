...
export interface OpenAIResponse {
  ...
  query: {
    range: 'today' | 'yesterday' | 'week' | 'month' | 'custom' | null;
    from: string | null;
    to: string | null;
    category: string | null;
  };
...
5. หากผู้ใช้ถามสรุป "เมื่อวาน" ให้ตั้ง query.range เป็น "yesterday"
6. การแยกประเภท (type):
...
7. สกัดข้อมูลจำนวนเงิน (amount), หมวดหมู่ (category), วันที่ (date), และหมายเหตุ (note)
8. วันที่ให้ใช้รูปแบบ ISO-8601 (Asia/Bangkok)
9. ห้ามตอบเป็นข้อความอื่นนอกจาก JSON
